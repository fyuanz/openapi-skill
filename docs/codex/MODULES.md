...
| `openapi-skill-node` | TypeScript npm library/CLI for one project Skill spanning explicitly configured services/documents and one or more output parents | `openapi-skill@2.1.2` is published as npm `latest`; local source passes 65 tests |
...
core/2 trees only for safe atomic replacement. Published npm `latest` `2.1.2` passes 65 local Node tests; the prior `2.1.1` packed tarball
