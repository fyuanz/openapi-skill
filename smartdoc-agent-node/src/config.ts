...
import { ResolvedDocumentSource, ResolvedServiceSource, ResolvedSmartDocConfig, SourceType } from './types.js';

export async function loadConfig(cwd: string, explicit?: string): Promise<ResolvedSmartDocConfig> {
  ...
  for (const candidate of candidates) {
    try {
      ...
      raw = candidate.endsWith('package.json') ? parsed.smartdocAgent : parsed;
      if (raw !== undefined) break;
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw new Error(`CONFIG: cannot read ${candidate}: ${message(error)}`, { cause: error });
    }
  }
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new Error(`CONFIG: no SmartDoc configuration found in ${candidates.join(' or ')}`);
  ...
  const services = hasServices ? parseServices(value.services) : [parseLegacyService(value, keywords)];
...
function parseServices(value: unknown): ResolvedServiceSource[] {
  ...
  const services = value.map((raw, index) => {
    ...
    return {
      ...
      documents: parseDocuments(service.documents, serviceId)
    };
  });
  return services.sort((a, b) => a.serviceId.localeCompare(b.serviceId, 'en'));
}

function parseLegacyService(value: Record<string, unknown>, keywords: string[]): ResolvedServiceSource {
  identity(value.serviceId as string);
  const serviceId = value.serviceId as string;
  return { serviceId, sourceType: 'internal', keywords, documents: parseDocuments(value.documents, serviceId) };
}

function parseDocuments(value: unknown, serviceId: string): ResolvedDocumentSource[] {
  ...
  const documents = value.map((raw) => {
    ...
    ids.add(id);
    if (typeof source.url !== 'string') throw new Error(`CONFIG: invalid URL for ${serviceId}/${id}`);
    let url: URL; try { url = new URL(source.url); } catch { throw new Error(`CONFIG: invalid URL for ${serviceId}/${id}`); }
    if (!['http:', 'https:'].includes(url.protocol)) throw new Error(`CONFIG: ${serviceId}/${id} URL must use http or https`);
    return { id, url: source.url, keywords: normalizeKeywords(source.keywords, `${serviceId}/${id}`) };
  });
