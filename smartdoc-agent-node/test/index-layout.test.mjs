...
test('emits core/3 semantic indexes and short readable file names', () => {
  const files = generateSkill({ serviceId: 'svc', skillName: 'svc-api', documents: new Map([['public', input]]) });
  assert.equal(JSON.parse(files.get('references/source.json')).generatorVersion, 'smartdoc-agent-core/3');
...
test('project mode retains searchable service indexes and core/3 provenance', () => {
