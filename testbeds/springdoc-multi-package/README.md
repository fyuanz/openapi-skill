...
| 当前 Skill ZIP | http://127.0.0.1:18080/openapi-skill/skill.zip |
...
7 个文档内 Schema、来源分组及 OpenAPI Skill 接口不会出现在生成契约中。`SampleEndpointTest` 验证样例业务接口。
集成脚本还断言 Maven 日志没有执行 Spring Boot `start/stop`、SpringDoc Maven 抓取或 OpenAPI Skill Maven goal，
并确认不会创建 `target/generated-openapi` 或 `target/generated-resources/openapi-skill`。

因此普通 `compile`、`test`、`package`、`verify` 不再为了 OpenAPI Skill 启动额外应用，也不生成临时 OpenAPI/Skill 文件。
...
真实项目若包含受限接口文档，应限制 `/openapi-skill/skill.zip` 的访问权限。
