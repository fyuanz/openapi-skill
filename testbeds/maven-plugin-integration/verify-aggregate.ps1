...
function Status([string] $root, [string] $id) {
    return (Get-Content -Raw -LiteralPath (Join-Path $root ".openapi-skill/status/$id.json") | ConvertFrom-Json)
}
...
Invoke-Maven 'reactor' $PSScriptRoot @('-B', '-Paggregate-skills', '-T', '2', "-Dopenapi.skill.output=$reactorOutput", 'compile')
...
Assert-That ($reactorLog.IndexOf('OpenAPI Skill [platform] SUCCESS') -gt $reactorLog.IndexOf('OpenAPI Skill [orders] SUCCESS')) 'aggregate ran before orders'
Assert-That ($reactorLog.IndexOf('OpenAPI Skill [platform] SUCCESS') -gt $reactorLog.IndexOf('OpenAPI Skill [billing] SUCCESS')) 'aggregate ran before billing'
...
$common = @('-B', '-f', 'skill-set/pom.xml', "-Dopenapi.skill.output=$bothOutput",
    "-Daggregate.orders.directory=$ordersInput", "-Daggregate.billing.directory=$billingInput")
Invoke-Maven 'both' $PSScriptRoot ($common + @('-Dopenapi.skill.output.mode=both', 'compile'))
foreach ($id in @('orders', 'billing', 'platform')) {
...
Invoke-Maven 'repeat' $PSScriptRoot ($common + @('-Dopenapi.skill.output.mode=both', 'compile'))
...
Invoke-Maven 'failed-member' $PSScriptRoot ($common + @('-Dopenapi.skill.output.mode=both', 'compile'))
...
Invoke-Maven 'recovered' $PSScriptRoot ($common + @('-Dopenapi.skill.output.mode=both', 'compile'))
foreach ($id in @('orders', 'billing', 'platform')) {
...
Invoke-Maven 'aggregate-only' $PSScriptRoot @('-B', '-f', 'skill-set/pom.xml', "-Dopenapi.skill.output=$aggregateOutput", 'compile')
...
Invoke-Maven 'service-only' $PSScriptRoot @('-B', '-f', 'skill-set/pom.xml', "-Dopenapi.skill.output=$serviceOutput", '-Dopenapi.skill.output.mode=service', 'compile')
