...
SpringDoc WebMVC applications should use `openapi-skill-spring-boot-starter` and its runtime ZIP endpoint, which does
...
<plugin>
  ...
  <configuration>
    ...
    <outputDirectory>${project.build.directory}/generated-resources/openapi-skill</outputDirectory>
...
`<skillName>/` and updater state under `.openapi-skill/`.
...
The default output parent is `${project.build.directory}/generated-resources/openapi-skill`; `clean` removes it. Never
