...
import io.github.fyuanz.openapi.skill.core.ServiceSkillUpdater;
...
public final class GenerateSkillMojo extends AbstractMojo {
    ...
    @Parameter(defaultValue = "${project.build.directory}/generated-resources/openapi-skill")
    ...
    public void execute() {
        ...
        try {
            ...
            if (configuredDocuments.isEmpty()) {
                getLog().info("OpenAPI Skill [" + serviceId + "] SKIPPED: no OpenAPI JSON files found in "
            ...
            String summary = "OpenAPI Skill [" + serviceId + "] " + result.outcome() + ": " + result.message()
            ...
            getLog().warn("OpenAPI Skill [" + service + "] FAILED: " + detail);
            getLog().debug("OpenAPI Skill update failure", error);
        }
    }
